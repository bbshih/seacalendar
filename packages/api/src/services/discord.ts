...
export const postEventToDiscord = async (
  ...
  if (!Config.discord.botToken) {
    logger.warn(
      "Discord bot token not configured, skipping Discord announcement",
    );
    return;
  }
  ...
  try {
    ...
    const response = await fetch(
      `${Config.discord.apiUrl}/channels/${channelId}/messages`,
      {
        method: "POST",
        headers: {
    ...
    if (!response.ok) {
      const error = await response.json();
      logger.error("Discord message post failed", { error, channelId });
    ...
    logger.error("Discord event posting error", {
      error,
      pollId: eventData.pollId,
    });
