...
describe('Google OAuth Service', () => {
  ...
  describe('exchangeCode', () => {
    ...
    it('should throw ApiError on token exchange failure', async () => {
      ...
      await expect(googleService.exchangeCode(authCode)).rejects.toThrow('Failed to exchange Google authorization code');
    });
  });
  ...
  describe('getUserInfo', () => {
    ...
    it('should throw ApiError on user info fetch failure', async () => {
      ...
      await expect(googleService.getUserInfo(accessToken)).rejects.toThrow('Failed to fetch Google user info');
    });
  });
  ...
  describe('refreshAccessToken', () => {
    ...
    it('should throw ApiError on refresh failure', async () => {
      ...
      await expect(googleService.refreshAccessToken(refreshToken)).rejects.toThrow('Failed to refresh Google access token');
    });
  });
  ...
  describe('createOrLinkAccount', () => {
    describe('creating new account', () => {
      ...
      it('should throw error if no refresh token received', async () => {
        ...
        await expect(googleService.createOrLinkAccount(authCode)).rejects.toThrow('No refresh token received');
      });
    ...
    describe('linking to existing account', () => {
      ...
      it('should throw error if Google account already linked to different user', async () => {
        ...
        await expect(googleService.createOrLinkAccount(authCode, existingUserId)).rejects.toThrow(ApiError);
        await expect(googleService.createOrLinkAccount(authCode, existingUserId)).rejects.toThrow('already linked to another user');
      });
    ...
    describe('token expiration', () => {
      it('should calculate correct expiration time', async () => {
        ...
        expect(expiresAt.getTime()).toBeLessThanOrEqual(afterTime + mockTokens.expires_in * 1000 + 1000);
      });
    });
  });
});
