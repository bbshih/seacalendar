...
import { PollType, PollStatus } from '@seacalendar/database';
...
describe('Poll Service', () => {
  ...
  describe('createPoll', () => {
    it('should create a poll with options', async () => {
      ...
      vi.mocked(prisma.poll.create).mockResolvedValue({
        ...mockPoll,
        options: [
          { id: 'opt-1', pollId: mockPoll.id, label: 'Option 1', description: null, date: new Date('2024-06-15'), timeStart: '18:00', timeEnd: '21:00', order: 0, createdAt: new Date() },
          { id: 'opt-2', pollId: mockPoll.id, label: 'Option 2', description: null, date: new Date('2024-06-16'), timeStart: '18:00', timeEnd: '21:00', order: 1, createdAt: new Date() },
        ],
        invites: [],
    ...
    it('should create a poll with invited users', async () => {
      ...
      vi.mocked(prisma.poll.create).mockResolvedValue({
        ...mockPoll,
        options: [{ id: 'opt-1', pollId: mockPoll.id, label: 'Option 1', description: null, date: null, timeStart: null, timeEnd: null, order: 0, createdAt: new Date() }],
  ...
  describe('getPoll', () => {
    ...
    it('should allow invited user to view poll', async () => {
      ...
      vi.mocked(prisma.poll.findUnique).mockResolvedValue({
        ...
        votes: [],
        invites: [
          { id: 'invite-1', pollId, userId, invitedAt: new Date(), user: createMockUser({ id: userId }) },
        ],
  ...
  describe('updatePoll', () => {
    ...
    it('should throw error if poll not found', async () => {
      ...
      await expect(pollService.updatePoll(pollId, userId, updateData)).rejects.toThrow('Poll not found');
    });
  ...
  describe('finalizePoll', () => {
    it('should finalize poll with valid option', async () => {
      ...
      vi.mocked(prisma.poll.findUnique).mockResolvedValue({
        ...
        status: PollStatus.VOTING,
        options: [
          { id: optionId, pollId, label: 'Option 1', description: null, date: null, timeStart: null, timeEnd: null, order: 0, createdAt: new Date() },
        ],
      } as any);
    ...
    it('should throw error if poll not found', async () => {
      ...
      await expect(pollService.finalizePoll(pollId, userId, optionId)).rejects.toThrow('Poll not found');
    });
    ...
    it('should throw error if user is not creator', async () => {
      ...
      vi.mocked(prisma.poll.findUnique).mockResolvedValue({
        creatorId: 'user-123',
        status: PollStatus.VOTING,
        options: [{ id: optionId, pollId, label: 'Option 1', description: null, date: null, timeStart: null, timeEnd: null, order: 0, createdAt: new Date() }],
      } as any);
    ...
    it('should throw error if invalid option ID', async () => {
      ...
      vi.mocked(prisma.poll.findUnique).mockResolvedValue({
        ...
        status: PollStatus.VOTING,
        options: [
          { id: 'option-1', pollId, label: 'Option 1', description: null, date: null, timeStart: null, timeEnd: null, order: 0, createdAt: new Date() },
        ],
      } as any);

      await expect(pollService.finalizePoll(pollId, userId, optionId)).rejects.toThrow('Invalid option ID');
    });
  });
  ...
  describe('getUserPolls', () => {
    ...
    it('should filter polls by status', async () => {
      const userId = 'user-123';
      const mockPolls = [createMockPoll({ id: 'poll-1', creatorId: userId, status: PollStatus.VOTING })];
