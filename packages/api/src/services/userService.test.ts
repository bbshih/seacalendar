...
import { PollStatus } from '@seacalendar/database';
...
describe('User Service', () => {
  ...
  describe('updateUser', () => {
    ...
    it('should allow updating phone to same phone', async () => {
      ...
      vi.mocked(prisma.user.findUnique).mockResolvedValue(
        createMockUser({ id: userId }) as any
      );
  ...
  describe('getUserPolls', () => {
    ...
    it('should include poll options, votes, and invites', async () => {
      ...
      const mockPoll = {
        ...createMockPoll({ id: 'poll-1', creatorId: userId }),
        options: [
          { id: 'opt-1', pollId: 'poll-1', label: 'Option 1', description: null, date: null, timeStart: null, timeEnd: null, order: 0, createdAt: new Date() },
        ],
        votes: [
          { id: 'vote-1', pollId: 'poll-1', voterId: 'user-456', availableOptionIds: ['opt-1'], maybeOptionIds: [], notes: null, createdAt: new Date(), updatedAt: new Date() },
        ],
        invites: [
          { id: 'invite-1', pollId: 'poll-1', userId: 'user-456', invitedAt: new Date() },
        ],
      };
