  ...
  .addSubcommand(subcommand =>
    subcommand
      ...
      .addStringOption(option =>
        option
          .setName('event_url')
          .setDescription('The event URL or ID')
          .setRequired(true)
      )
      .addStringOption(option =>
        option
      ...
      .addStringOption(option =>
        option
          .setName('content')
          .setDescription('Your memory text or photo URL')
          .setRequired(true)
      )
  )
  .addSubcommand(subcommand =>
...
async function handleAdd(interaction: ChatInputCommandInteraction) {
  ...
  try {
    ...
    await interaction.editReply({
      content: `❌ Failed to add memory: ${error instanceof Error ? error.message : 'Unknown error'}`,
    });
  }
}
...
async function handleView(interaction: ChatInputCommandInteraction) {
  ...
  try {
    ...
    if (memories.length > 10) {
      embed.setFooter({ text: `Showing 10 of ${memories.length} memories. View all at cal.billyeatstofu.com` });
    }

    await interaction.editReply({ embeds: [embed] });
  } catch (error) {
    console.error('Error viewing memories:', error);
    await interaction.editReply({
