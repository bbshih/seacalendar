...
function getPhotosClient(): any {
  ...
  try {
    ...
    console.error('Failed to initialize Google Photos client:', error);
...
export async function createSharedAlbum(title: string): Promise<CreateAlbumResult> {
  if (!isConfigured()) {
    throw new Error('Google Photos API not configured. Set GOOGLE_APPLICATION_CREDENTIALS env var.');
  }
