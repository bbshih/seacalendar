...
export async function createEventPoll(input: CreatePollInput): Promise<PollWithDetails> {
  ...
  const deadline = input.votingDeadline || new Date(Date.now() + Config.bot.defaultVotingDeadlineDays * 24 * 60 * 60 * 1000);
 ...
 */
export async function getGuildPolls(guildId: string, status?: PollStatus): Promise<PollWithDetails[]> {
