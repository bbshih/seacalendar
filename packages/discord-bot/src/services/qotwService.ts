 ...
 */
export async function getQuestionsForPoll(guildId: string, count: number = 5): Promise<QotwQuestion[]> {
 ...
 */
export async function getQuestion(questionId: string, guildId: string): Promise<QotwQuestion | null> {
  return prisma.qotwQuestion.findFirst({
    where: {
      id: questionId,
      guildId,
 ...
 */
export function canModifyQuestion(question: QotwQuestion, userId: string, isAdmin: boolean): boolean {
 ...
 */
export async function setNextQuestion(guildId: string, questionId: string | null): Promise<QotwConfig> {
